    ...
    <div className="site-shell">
      ...
      <main className={`dashboard-layout ${selectedSlug ? "has-selection" : ""}`}>
        <section ref={listRef} className="project-list" aria-label={locale === "en" ? "Projects" : "Proyectos"}>
          ...
          {filtered.map((project) => <ProjectCard key={project.id} project={project} selected={project.slug === selectedSlug} locale={locale} t={t} onOpen={openDetail} />)}
        </section>
        {selectedSlug && <ProjectPanel ref={detailRef} project={detailSlug === selectedSlug ? detail : null} error={detailSlug === selectedSlug && detailError} locale={locale} t={t} onClose={closeDetail} />}
      </main>
...
function ProjectCard({ project, selected, locale, t, onOpen }: { project: ProjectSummary; selected: boolean; locale: Locale; t: ReturnType<typeof getMessages>; onOpen: (slug: string, trigger: HTMLElement) => void }) {
  return <article className={`project-card ${selected ? "is-selected" : ""}`} style={accentStyle(project.accentColor)}>
    <OperationalBanner project={project} locale={locale} t={t} />
...
interface ProjectPanelProps { project: ProjectDetail | null; error: boolean; locale: Locale; t: ReturnType<typeof getMessages>; onClose: () => void }

const ProjectPanel = forwardRef<HTMLElement, ProjectPanelProps>(function ProjectPanel({ project, error, locale, t, onClose }, ref) {
  return <><button className="detail-backdrop" aria-label={t.close} onClick={onClose} /><aside ref={ref} className="project-panel" style={project ? accentStyle(project.accentColor) : undefined} tabIndex={-1} role="dialog" aria-modal="true" aria-label={project?.title ?? t.overview} onKeyDown={(event) => trapFocus(event)}>
    ...
    {error ? <div className="panel-loading"><p>{t.unavailable}</p></div> : !project ? <div className="panel-loading"><span className="loading-ring" /><p>{locale === "en" ? "Loading the full story…" : "Cargando todos los detalles…"}</p></div> : <div className="panel-content" key={project.id}>
      <OperationalBanner project={project} locale={locale} t={t} panel />
...
function OperationalBanner({ project, locale, t, panel = false }: { project: ProjectSummary; locale: Locale; t: ReturnType<typeof getMessages>; panel?: boolean }) {
